...
\swp{Suggestion for the entire manuscript: I feel like it could be easier to read if we keep the ordering of (speed and strength) or (strength and speed). I wonder if we should says strength and speed, given that we almost always introduce strength first and then there's a corresponding speed? Not necessary but just a thought... If you think this is helpful, I can do this in no time.}
...
We then show that we can likewise measure an intervention's ``speed'', and that there is a duality between the fact that the threshold $\RR=1$ determines the effective strength of intervention required for disease elimination, and the fact that the threshold $r=0$ determines the effective speed of intervention required for elimination. 
\swp{This sentence is a bit long. How about we break it into two sentences?}
\swp{
We then show that we can likewise measure an intervention's ``speed'' on the same scale as the growth rate.
There is a duality between the thresholds $\RR=1$ and $r=0$, which determine the effective strength and speed of intervention required for disease elimination.
}
...
Imagine a control measure that proportionally reduces $K$ \swp{by a factor of $\theta$}, for example, by protecting a fixed fraction of susceptibles through vaccination (\figref{constant}A). We then have:
\begin{equation}
...
Modelers typically rely on mechanistic models (often using ordinary differential equations models) to model disease spread and try to determine intervention parameters, such as vaccination proportion, to prevent an outbreak.
...
\swp{Need a concluding sentence?}
...
More early transmission, for a given value of \rr, means \swp{shorter generation intervals and therefore} less transmission per individual -- that is, a lower value of \RR. Thus, although earlier transmission could make intervention less effective, it also means that less intervention may be necessary. For interventions like lockdowns, which are generally assumed affect everyone roughly equally, the strength-based perspective thus gives a clear (and somewhat surprising) answer: more early transmission means we will conclude that non-targeted interventions are \emph{more} effective.

For interventions which \swp{target} infected individuals, like contact tracing or test-based isolation, the speed-based paradigm provides clearer insight into the likely effects of early transmission. 
