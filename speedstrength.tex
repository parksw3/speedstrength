...
\swp{I also changed the title to strength and speed}
...
As an example, we show that evaluating the prospects of ``test-and-treat'' interventions against the human immunodeficiency virus (HIV) can be done more clearly on the strength than speed scale, given uncertainty in the proportion of HIV spread that happens early in the course of infection. 
...
\begin{equation}
	...
	\eqlab{strengthMean}
\end{equation}
Again, we have that the disease cannot spread when $\theta \geq \RR$. 
...
\swp{
We note that intervention function $L$ and the strength of intervention $\theta$ do not need to be calculated explicitly to determine elimination in many contexts---rather, they are abstractions of the current epidemic modeling practices.
Modelers typically rely on mechanistic models (often based on ordinary differential equations) to model disease spread and try to determine intervention parameters, such as vaccination proportion, to prevent an outbreak.
By doing so, they make implicit assumptions about the shape of $L$ and therefore $\theta$.
In these cases, the strength of intervention corresponds to the ratio of the reproduction numbers for the null model without intervention and the model with intervention.
}
...
\swp{
Like intervention strength $\theta$, intervention speed $\phi$ is also an abstraction---
that is, the mechanistic models of interventions make implicit assumptions about the shape of the hazard rate $h$ and therefore $\phi$.
The speed of intervention then corresponds to the difference between the observed growth rate and the growth rate of a simulated epidemic under intervention.
}

The disease cannot spread when $\phi \geq r$. In other words, \rr, like \RR, is a metric for how hard a disease is to eliminate: we can measure the ``speed'' of a control measure, this must exceed the speed of the disease (\rr) to achieve elimination.
...
\jd{The above is an old comment. I'm not currently loving the idea of including that figure, but I guess we should make it and see?}
...
There has been a great deal of discussion of the importance of pre-symptomatic transmission of COVID-19. 
...
More early transmission, for a given value of \rr, means {shorter generation intervals and therefore} less transmission per individual -- that is, a lower value of \RR. Thus, although earlier transmission could make intervention less effective, it also means that less intervention may be necessary. For interventions like lockdowns, which are generally assumed affect everyone roughly equally, the strength-based perspective thus gives a clear (and somewhat surprising) answer: more early transmission means we will conclude that non-targeted interventions are \emph{more} effective.
...
Thus, if the hazard of control is increasing through time, as might be expected with isolation of symptomatic cases, more early transmission means that the effective speed of control is lower than originally thought, and control will be harder.
Conversely, if the hazard of control decreases through time, as might be expected with isolation of returning travellers, or event-based contact tracing, more early transmission means that control will be easier than originally thought. \swp{I think this part needs to be explained more clearly. If hazard decreases through time, then the intervention focuses on reducing early transmission (whereas before, we try to reduce late transmission). So if there is more early transmission, than easier to control. I also wonder if it's better to frame it in terms of symptom-based intervention vs non-symptom-based? Increasing/decreasing hazard may be a bit vague.}
...
%% \jd{What does this comment mean?}
\swp{---the overall conclusion can change whether we calibrate our models to \rr or \RR.}
\swp{I feel like this is important but I'm not sure if this is a good place to put it. I think we need to say it explicitly.}

\swp{A new suggested paragraph. I think some version of this paragraph may help deal with above sentence (the image you sent me saying that you don't understand). I think it can also help deal with the reviewer's comments about the growth phase}
\swp{
Here, we focused on the initial growth phase an epidemic, assuming that the observed growth rate \rr is fixed throughout our analysis.
For endemic diseases, as discussed previously, strength-based framework can provide a clearer perspective---therefore, it makes more sense to rely on fixed \RR when evaluating the difficulty of elimination.
In these cases, parameter uncertainty will generally not affect the efficacy of strength-like interventions but can affect conclusions about speed-lik interventions:
given \RR, shorter generation intervals imply faster epidemic \citep{eaton2014proportion, powers2014impact, park2019practical}, requiring faster intervention to control .
While both strength- and speed-based frameworks will give the same answers for a given set of assumptions about an epidemic,
these conclusions can depend on whether we calibrate our models to the strength or speed of an epidemic.
}
...
\swp{Do we need to explain more? feels a bit abrupt.}
...
For example, only 1 out of 7 preliminary analyses of the COVID-19 outbreak that were published as preprints between January 23--26, 2020 reported the doubling time of an epidemic \citep{bedfordncov, imaincov, liuncov, majumderncov, readncov, riouncov, zhaoncov}. \swp{does this feel weak now? Maybe need something like ``Many subsequent studies then relied on strength-based frameworks to evaluate the efficacy of speed-like interventions, such as contact tracing, with a few exceptions'' with citations?}
