...
While an epidemic can be characterized by its speed (i.e., the exponential growth rate $r$) and strength (i.e., the reproduction number \RR), the effectiveness of an intervention strategy is often, if not always, determined by its ability to reduce \RR\ below 1.
...
We thus showed that the standard paradigm for \Rx\ and control has a natural parallel interpretation in terms of $r$.
...
Whether strength- or speed-based perspective is more appropriate depends on the disease as well as the intervention strategies.
For newly invading pathogens, we expect the speed-based perspective to be almost always better.
When an epidemic is growing exponentially, the basic reproductive number cannot be estimated with confidence \citep{weitz2015modeling}, especially when there is large uncertainty in the shape of the generation-interval distribution \citep{park2020reconciling}.
