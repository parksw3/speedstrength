...
The effectiveness of an epidemic intervention is often measured by its ability to reduce the reproductive number -- \Rx, or outbreak ``strength'' -- below 1. The exponential growth rate -- $r$, or outbreak ``speed'' -- is often seen just as a stepping stone to \Rx\, or even overlooked entirely (ParkPreprint for examples?).
We argue that \Rx\ and $r$ provide equally valid, complementary perspectives on epidemic control, and that there are situations where each provides a clearer picture than the other.

In this study, we:
first extended the standard paradigm of \Rx\ as critical parameter for control, by defining the strength of an intervention on the same scale as \Rx, the strength of the epidemic; 
...
We thus showed that the standard paradigm for \Ro\ and control has a natural parallel interpretation in terms of $r$. 
...
\jd{I don't like this as a close; maybe we have to mention some of it \ldots.}
\jd{We definitely need to cite pasztor1996r0. I'm less interested in the Bull and MacArthur evolutionary stuff, which seems tangential to me. I wonder what's a good thing to cite for the over-emphasis on big R? I kind of feel like the stampede to estimate big R while looking past little R in Ebola and CovID should be part of our story here.}
