...
JD to do a condom calculation framing.
...
Here, we generalize the idea of a threshold for successful intervention by measuring an intervention's strength on the same scale as the reproductive number \swp{(and by measuring an intervention's speed on the same scale as the initial exponential growth rate)}. 
...
\swp{Need values}
First, consider a condom intervention that is approximately 80\% effective in reducing HIV seroconversion \citep{weller2002condom}.
