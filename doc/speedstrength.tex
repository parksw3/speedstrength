...
An epidemic can be characterized by speed (i.e., the exponential growth rate $r$) or strength (i.e., the reproductive number \RR).
Disease modelers have historically placed much more emphasis in strength, in part because the effectiveness of an intervention strategy is typically evaluated on this scale.  
Here we develop a mathematical framework for this classic, strength-based paradigm and show that there is a corresponding speed-based paradigm which can provide complementary insights.
...
We argue that, just as the strength-based paradigm provides the clearest insight into certain questions, the speed-based paradigm provides the clearest view in other cases.
\swp{We argue that the speed-based paradigm provides the clearest insight into certain questionsm just as the strength-based paradigm provides view in other cases.}
As an example, we show that evaluating the prospects of ``test-and-treat'' interventions \swp{(i.e., identifying infected individuals and putting them to clinical care, such as antiretroviral therapy)} against the human immunodeficiency virus (HIV) can be done more clearly on the strength than speed scale, given uncertainty in the proportion of HIV spread that happens early in the course of infection. \swp{you mean speed than strength, right?}
We suggest that disease modelers should refrain from putting too much emphasis on the reproductive number at the expense of the exponential growth rate, but instead look at these as complementary approaches.
...
Here, we show that $r$ can also serve as a threshold, and also provide a useful metric for difficulty of elimination. We first generalize the idea that \RR\ measures the difficulty of elmination by showing we can measure an intervention's ``strength'' on the same scale as the reproductive number. 
...
On the other hand, consider a ``test-and-treat'' strategy in which infected individuals are identified and put to clinical care, such as antiretroviral therapy (ART) \citep{garnett2009treating, granich2009universal, nah2017test}.
We assume that the rate of identification and removal is initially constant \swp{is approximately constant in the beginning}. 
Thus, the proportion continuing to transmit declines \swp{approximately} exponentially, and the effect of intervention $\tsub{L}{test}$ increases \swp{approximately} exponentially.
However, we assume that removal eventually saturates, to account for the effects of people who avoid identification, persistent treatment failures, and the possibility of rare transmission even under effective treatment
(\figref{test}A; see caption for the functional form of $\tsub{L}{test}$).
Under this assumption, the strength of intervention decreases with the proportion of early transmission in a manner roughly similar to the epidemic strength (\figref{test}B).
...
the assumed hazard rate goes down after a few months to reflect the saturation assumptions discussed above (\figref{test}C). 
...
The intervention speed increases slightly as proportion of early transmission increases because the subpopulation that the intervention fails to reach become relatively more important if late transmission is more important (\figref{test}D).
...
\swp{can we combine the first two paragraphs into one paragraph? The second paragraph is just 2 sentences long...}
...
While both speed- and strength-based frameworks give the same conclusion about the outcome of an intervention, sometimes one provides a clearer understanding of the outcome than the other.
...
Some practical questions lend themselves more naturally to one approach or another. For example, in the classic case of vaccination to eliminate a previously established childhood disease, both disease spread and intervention can be clearly characterized using strength \citep{anderson1985vaccination}. In our HIV example, both the HIV epidemic and the test-and-treat intervention can be clearly characterized using speed. Other cases, such as social distancing (strength-like) in the early stages of COVID-19 (clear speed) may not fit so neatly into either paradigm, however.
