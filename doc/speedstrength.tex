...
Much research has prioritized estimates of \RR, and particularly its value in a fully susceptible population, called \Ro, because \RR\ has a threshold value (i.e., $\RR=1$) that determines whether a disease can invade, the level of equilibrium, and the effectiveness of control efforts \citep{anderson1991infectious, diekmann1990definition}.
...
$\tsub{p}{early} = 0.23$.  We assume that the initial speed of the epidemic is $r=0.452\,\mathrm{year}^{-1}$; \citep{barron2013eliminating} (see \figref{example}B), and ask what value of $\Ro$ would produce this rate of growth.
...
The effectiveness of an epidemic intervention is often measured by its ability to reduce the reproductive number -- \Rx, or outbreak ``strength'' -- below 1. The exponential growth rate -- $r$, or outbreak ``speed'' -- is often seen just as a stepping stone to \Rx\, or even overlooked entirely \citep{park2020reconciling}.
...
Even though the contrast between two perspectives was introduced in ecology more than 20 years ago, it is still rare in epidemiology.
...
For example, only 1 out of 7 preliminary analyses of the COVID-19 outbreak that were published as preprints between January 23--26, 2020 reported the doubling time \citep{bedfordncov, imaincov, liuncov, majumderncov, readncov, riouncov, zhaoncov}.
When an epidemic is growing exponentially, however, the basic reproductive number may not be identifiable \citep{taylor2016stochasticity}.
