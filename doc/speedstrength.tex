...
Epidemics can be characterized by speed (i.e., the exponential growth rate $r$) or strength (i.e., the reproductive number \RR).
...
As an example, we show that evaluating the prospects of test-and-treat interventions against HIV can be done more clearly on the strength than speed scale, given uncertainty in the proportion of HIV spread that happens early in the course of infection. 
...
For interventions, we expect the strength-based framework to be clearer for evaluating intervention strategies that seek to reduce the overall transmission at the population level, while the speed-based framework may be clearer for interventions that work at the individual level, like test-and-treat for HIV, or contact-tracing and quarantine for COVID-19.
In other cases, such as real-time rollout of vaccines during an outbreak, both speed and strength approaches might be similarly uncertain because the result depends both on the speed of the rollout and the (strength-like) final coverage.

Some practical questions lend themselves more naturally to one approach or another. For example, in the classic case of vaccination to eliminate a previously established childhood disease, both disease spread and intervention can be clearly characterized using strength. In our HIV example, both the HIV epidemic and the test-and-treat intervention can be clearly characterized using speed. Other cases, such as social distancing (strength-like) in the early stages of COVID-19 (clear speed) may not fit so neatly into either paradigm, however.
